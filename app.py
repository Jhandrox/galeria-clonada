from flask import Flask, render_template, request, redirect
from werkzeug.utils import secure_filename
import os
from pymongo import MongoClient
from bson import ObjectId 
from flask_mail import Mail, Message

EXTNS = ["png", "jpg", "jpeg"]
app = Flask(__name__)
app.config["UPLOAD_FOLDER"] = "./static/fondos"

client = MongoClient("mongodb://127.0.0.1:27017")
#basededatos
db = client.fondos_flask
#tabla(en mongo)
misfondos = db.fondos
#config  email
app.config["MAIL_SERVER"]= "mail.cepibase.int"
app.config["MAIL_PORT"] = 25
app.config["MAIL_USERNAME"] = "USUARIO"
app.config["MAIL_PASSWORD"] = "PASSWORD"
app.config["MAIL_USE_TLS"] = False
app.config["MAIL_USE_SSL"] = False
mail= Mail(app)

def archivosAceptados(nombre):
    return "." in nombre and nombre.rsplit(".", 1)[1] in EXTNS

@app.route("/", methods=["GET", "POST"])
@app.route("/galeria")
def indexhtml():
    tema=request.values.get("tema")
    estilos={}
    if tema == None:
        dbfondos=misfondos.find()
        estilos["todos"]="active"
        estilos
        print(estilos,"\n<--")

    else:
        dbfondos=misfondos.find({"tags": {"$in":[tema]}})
        estilos[tema]="active"  
        print(estilos,"\n<--else")

    return render_template("index.html",activo=estilos,lista=dbfondos,)

@app.route("/aportar")
def aportar():
    return render_template("aportar.html")

@app.route("/form_email")
def formEmail():
    id=request.values.get("_id")
    imgs=misfondos.find_one({"_id":ObjectId(id)})
    return render_template("form_email.html", id=id, fondo=imgs["fondo"],
     titulo=imgs["titulo"], descripcion=imgs["descripcion"])


@app.route("/insertar", methods=["POST"])
def insertar():
    f = request.files["archivo"]
    if f.filename == "":
         return render_template("aportar.html",mensaje="Hay que indicar un archivo de fondo")
    else:
        if archivosAceptados(f.filename):
            archivo = secure_filename(f.filename)
            f.save(os.path.join(app.config["UPLOAD_FOLDER"], archivo))
        else:
             return render_template("aportar.html",mensaje="¡El archivo indicado no es una imagen!")

    tit = request.values.get("titulo")
    desc = request.values.get("descripcion")
    tags=[]
    if request.values.get("animales"):
        tags.append("animales")
    if request.values.get("naturaleza"):
        tags.append("naturaleza")
    if request.values.get("ciudad"):
        tags.append("ciudad")
    if request.values.get("deporte"):
        tags.append("deporte")
    if request.values.get("personas"):
        tags.append("personas")
    misfondos.insert({"titulo":tit, "descripcion":desc, "fondo": archivo, "tags":tags})
    return redirect("/")

@app.route("/email", methods=["POST"])
def enviaemail():
    id=request.values.get("_id")
    foto=misfondos.find_one({"_id":ObjectId(id)})
    msg = Message("Fondos de pantalla Flask", sender = "alumno@cepibase.int")
    msg.recipients = [request.values.get("email")]
    msg.body = "Este es el fondo de pantalla seleccionado de nuestra galería."
    msg.html = render_template("email.html", titulo=foto["titulo"], descripcion=foto["descripcion"])
    with app.open_resource("./static/fondos/" + foto["fondo"]) as adj:
        msg.attach(foto["fondo"], "image/jpeg", adj.read())
    mail.send(msg)
    return redirect("/")

if __name__ == "__main__":
    app.run(debug=True)


